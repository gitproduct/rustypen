...
Congrats you have created a graphql api.
4. Now to build the graphql sever we have to install graphql library. use ```npm i graphql```.
5. Now create a schema, root resolver and query to fetch the data.
```
...
6. Access localhost:3000/api/graphql and you see the response. And to make the server listen the api body assign ``` const query to req.body.query```;
